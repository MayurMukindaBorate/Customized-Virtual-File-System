...
void man(char *name)
{
    ...
    {
        printf("Description: Used to deisplay information of file\n");
...
int ReadFile(int fd, char *arr, int isize)
{
    ...
    if(UFDTArr[fd].ptrfiletable->readoffset == UFDTArr[fd].ptrfiletable->ptrinode->permission != READ && UFDTArr[fd].ptrfiletable->ptrinode->permission != READ+WRITE)  return -2;
    ...
    while(1)    // Unconditional loop
    {
        ...
        else if(count == 2)
        {
            if(strcmp(command[0],"stat") == 0)
            {
                ...
                if(ret == -1)
                    printf("ERROR : Inorrect parameters\n");
                if(ret == -2)
